...
\subsection{Docker}
Given the technologies previously listed, our current system infrastructure looks like Figure \ref{fig:maveninfra}. We do a nice abstraction in terms of Maven which is building the compiler components for us, however, in order to develop or deploy this system the host machine would require many bits of software. This includes: PHP (and associated PHP package manager Composer), Apache, Laravel, Java and Maven. Obviously, this is quite a few things to download and would be a pain to develop across multiple machines and would make the deployment process complicated.
...
With Docker implemented, we present the final system architecture of the FunCompiler, as demonstrated in Figure \ref{fig:finalinfra}.
\begin{figure}[h]
\centering
