...
\begin{description}
\item Line 1: utilises Java 8 lambda expressions to define a POST route at the url ``/''. Within this expression we also declare the callback variables \texttt{req} and \texttt{res}, where \texttt{req} holds the contents of the request, and \texttt{res} will hold the contents of the response
\item Line 2: sets the \texttt{Content-Type} header of the response to be JSON.
\item Lines 3 and 4: extracts the values of the request's query parameters (as previously mentioned) and store them in \texttt{String} variables.
\item Line 5: converts the input program, currently stored as a \texttt{String}, into an \texttt{InputStream}, which is the typical format of input to ANTLR-based compilers.
\item Line 6: compiles the program via the \texttt{execute()} method defined within the FunRun module. As arguments we supply the \texttt{InputStream} along with the animation type. The return value of the \texttt{execute()} method is an object of the FunResponse class, which contains information collected during the compilation of the program.
\item Line 7: utilises a Java library named GSON which permits to conversion of the Java objects (and the data contained within) into JSON objects.
\end{description}

\subsection{Compiler Output \& The FunResponse Class}
As discussed, we are required to modify the compiler's implementation to enable the collection of various details relevant to the compilation of a program, these details will be sent to the web-application and used to build an animation of the compilation process. To accomplish this, we use a class named FunResponse. For each program that we receive from the web-application, we instantiate a new object of this class. This object is populated as the compilation proceeds, and is returned upon completion of the compilation. The information that we desire to collect is stored within the instance variables of this object, the declarations of which are given in the following code snippet (with access modifers removed):
\begin{lstlisting}
int numSyntaxErrors;
...
In the interest of brevity we will not go into the details of how the values of these variables are derived, however, we will analyse the contents of these variables and consider how they can be utilised by the web-application to build an animation. 
...
\item A node value, representing the text to be visually displayed within the node on the AST (in most cases, this is the same as the node name).
\end{itemize}
...
\end{lstlisting}
Within this example we can see that we have three different types of node, a \texttt{DIV}, an \texttt{ID} and a \texttt{NUM}. By examining the ID of the \texttt{DIV} node and the parent ID of the \texttt{ID} and \texttt{NUM} nodes, it is clear to see that \texttt{ID} and \texttt{NUM} are children of \texttt{DIV}. We can therefore apply this knowledge to build a visual representation of the AST that is represented by the original flat array, as illustrated in Figure \ref{fig:treeNodes}.
\begin{figure}[h]
\centering
...
Again, we see that each node contains an ID field, the value of which corresponds directly to the ID of a node in the \texttt{treeNodes} array. The node illustrated in the above example has the same ID and thus an explicit link to the \texttt{DIV} node we saw in the \texttt{treeNodes} array. The relevancy of this link is that the order of the nodes in \texttt{nodeOrder} dictate the order in which the nodes in \texttt{treeOrder} should be animated within the web-application. The animator is able to iterate though the nodes in \texttt{nodeOrder} and for each consecutive ID that is read, this informs the animator that the node within the AST that has the same ID is next to be highlighted.

%Continue from here.
We see also that we have fields for "explanations" and "table". These fields represent some of the augmentations that were discussed in Chapter 5. When the animator highlights the node in the AST linked via the ID field, it simultaneously uses the value of these fields to display the appropriate augmentations alongside the animation. We discussed earlier that the web-application must pass \texttt{type} as a query parameter, dictating whether we are animating contextual analysis or code generation. It is the value of this parameter that determines what augmentations are supplied to the \texttt{nodeOrder} array. In the above example we only have access to explanatory messages and type table information, meaning we must have chosen to animate contextual analysis. If we had however chosen to animate code generation, we would notice that "table" would contain information representing an address table and that a field for "objectCode" would be inserted.
