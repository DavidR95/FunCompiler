...
\begin{figure}[h]
\centering
\includegraphics[height=2.5cm,width=14cm]{images/token_stream.png}
...
Address allocation decides the representation and address of each variable in the source program. If the code generator encounters a variable declaration as it is traversing over the AST, it determines a suitable address for that identifier and inserts both values along with the corresponding scope into the address table. 

Code selection selects and generates the object code. Upon successful completion of this final phase, the compiler will have often produced an executable program.
