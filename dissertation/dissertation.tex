...
This chapter analyses the system architecture of the FunCompiler. That is, we build a model that specifies the structure and organisation of the system, alongside defining the communication patterns between distinct components within the infrastructure. We begin by discussing the differences between a monolithic and a micro-service architecture in regards to web-applications. We then consider which of these architectures the FunCompiler is to incorporate and how the components of the system will be arranged to fit that model. Finally, we use a bottom-up approach to describe the specific technologies that will be used within the FunCompiler, starting with bottom-level development frameworks up to top-level build tools and packaging systems.
\section{Monolithic vs. Micro-service Architecture}
Traditionally, websites have been built as large, monolithic applications, where each unique service within the website (e.g., authentication, routing, API exposure) is encapsulated under one application, as illustrated in Figure \ref{fig:mono}. Whilst a monolithic architecture may be an appropriate choice for many applications, if the system has a diverse range of services (of which each may have contrasting implementation logic), it often leads to each service being tightly coupled, unscalable and restricted in terms of programming language or development framework. 

The emerging trend is to extract each of these services out of the main application and allow them to behave as stand-alone components, referred to as micro-services. Figure \ref{fig:micro} demonstrates how a user interacts with the web-application, which in turn communicates with the individual micro-services that have been separated into their own components. Employing a micro-service architecture decouples the services, handles scalability issues more gracefully and allows a greater diversity in programming language and framework selection.
\begin{figure}[h]
	\centering
	\begin{subfigure}[b]{0.3\textwidth}
...
As discussed in Chapter 3, the compiler of the Fun language is a pre-existing piece of software built with Java and ANTLR. In order to incorporate the compiler with a web-application we have three choices:
\begin{enumerate}[label=\alph*)]
\item Translate the compiler into the same language used to implement the web-application. 
\item Implement the web-application using a Java-based framework.
\item Partition the FunCompiler into two distinct units: the web-application and the compiler, employing a micro-service architecture.
\end{enumerate}
Although all choices do essentially produce the same result, choice a) leads to a significant time investment that may produce a weaker implementation of the compiler, and choice b) compels us to make a framework selection based on a fragile justification which results in compounding the issues of tight coupling. Choice c) is the most rational optional which provides a sound separation of concerns, meaning the compiler does not need to be translated, nor are there any limitations on the choice of programming language or framework when implementing the web-application.

Following this, we henceforth consider the FunCompiler as two separate components: the web-application and the compiler. The web-application is the user-facing component, serving the web-pages and displaying the animation. The compiler behaves as a micro-service which takes a program as input, compiles it, and outputs the result of the compilation (the contents of this result we will discuss more in section ). 
...
With the FunCompiler split into its constituent components, we discuss the various system-level technologies and tools that shall be used to build and physically implement the system. First, we consider the tools used to develop the web-application, second, the tools used to develop the compiler and finally we discuss how we create an abstraction of the entire system and package it as a single portable instance.

\subsection{Web-application Technologies}
\subsubsection{Laravel}
A helpful facet of the FunCompiler is that the system requires no persistent storage, meaning we don't need to include a database. Despite lacking a database, the web-application does require some server side (or back-end) functionality, namely delivering API requests and handling the responses. This functionality is implemented using Laravel. Laravel is an open-source PHP framework intended for the rapid development of web-applications. Like the majority of back-end frameworks, Laravel follows a standard model-view-controller (MVC) architectural pattern. 
...
Composer is the quintessential dependency manager for PHP applications. Within our project we can declare all the dependencies our application requires, including API libraries and Laravel itself. Composer can either automatically install the latest versions of these libraries, or be locked into installing a specified version, appropriate for production-ready applications.
...
\subsubsection{Maven}
%Make less shit Say similar to composer it pulls deps but it also builds...
...
\subsection{Docker}
Given the technologies previously listed, our current system infrastructure looks like Figure \ref{fig:maveninfra}. We consider that if we were to attempt to develop the FunCompiler on a different machine, or perhaps deploy the FunCompiler, the host would be required to install several pieces of software. This includes PHP, Composer, Apache, Laravel, Java and Maven. Clearly, this is both quite a burdensome request and indicative that the code-base is not particularly easy to transport.
...
Similarly to how we used Maven to retrieve all the dependencies of the compiler and package it into a single component, we desire a tool which can retrieve the dependencies for the entire project and automatically execute a build. Docker is a tool designed to make it easier to create, deploy, and run applications by using containers. Before using a container we must define an image. Within the image we can state exactly what dependencies we require from the operating system to specific applications. The image is effectively a template of the environment we wish to run and when we run an image, we get a container (a container can be thought of as an instance of an image), as illustrated in Figure \ref{fig:docker1}. 
...
Whilst we won't delve into specifics, the FunCompiler creates two images: one for the web-application, or for the compiler. When these images are run (and thus converted into containers), they automatically retrieve all the dependencies they each need and execute any necessary builds. We then utilise Docker Compose, which is a tool for running multi-container applications. We define a short script which informs Docker Compose to build both containers if necessary, specifies the ports over which they will communicate and finally starts up both the Laravel server (in order to boot up the web-application) and the Spark server (in order to boot up the compiler). Upon execution of this script, the entire application will be running and available. 

There are a litany of advantages to use Docker but the take home point is that if another developer (or perhaps the system admin of the server deploying the application) was to obtain the source code for the FunCompiler, aslong as they had Docker installed, could build and run the FunCompiler with one command, without any of the application's dependencies installed locally. It is perhaps valuable to consider Docker container's as miniature virtual machines.
