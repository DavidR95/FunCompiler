...
The application will allow users to supply programs written in the Fun language as input to the Fun compiler. The user can then choose to animate either the contextual analysis or code generation phase of the compiler. In order to demonstrate how the compiler traverses the AST during these phases, nodes within the tree will be highlighted consecutively corresponding to the progression of the traversal, similar to the method displayed in Figure \ref{fig:slideshow}. In general, the application should:
\begin{itemize}
...
This chapter aims to briefly introduce the reader to the main tools and concepts used throughout this paper. We begin with an small overview of the Fun programming language and its capabilities. Then we consider at how and why compilers parse source code into syntax trees and why some representations of a syntax might be more beneficial than others. After that, we take an in-depth look at the three main phases of compilation. Finally, we discuss the use of code templates as a theoretical tool for assisting compiler developers in generating the object code or assembly language for various constructs within a programming language.
...
Fun has variables, procedures and functions. Variables can have a declared type of \texttt{int} or \texttt{bool} only. All variables must be initialised with an expression of the same type upon declaration. Procedures have type \texttt{T $\rightarrow$ void} and functions have type \texttt{T $\rightarrow$ T'}. T represents the type of an optional parameter. Procedures return no value (hence \texttt{$\rightarrow$ void}) and functions must return a value of the type T' (hence \texttt{$\rightarrow$ T'}). Fun has one predefined function and one predefined procedure. \texttt{read()} is a function that returns user input as an integer. \texttt{write(\textbf{int} n)} is a procedure writes the value of \texttt{n} to standard output. Below is an example Fun program which utilises variables, functions and procedures to calculate and write the factorial of user submitted input:
\begin{alltt}
...
Lexing is the process of breaking down an input program into a stream of \textit{\textbf{tokens}}. A token is simply a single element of the input program. For example, a token could be an individual identifier, operator or keyword. How the compiler chooses to define tokens is dependent upon the implementation of the language. The token stream is then passed as input to the parser. Figure \ref{fig:token-stream} shows how a small excerpt of Fun code may be decomposed into a token stream.

\begin{figure}[h]
\centering
\includegraphics[scale=0.5]{images/token_stream.png}
\caption{Lexing of Fun code into a token stream}
...
Scope checking ensures that every variable used in the program has been previously declared. If the contextual analyser encounters the \textit{declaration} of an identifier as it is traversing over the AST, it inserts the identifier along with its scope and type into the type table. If the contextual analyser finds that there is already an entry in the table with the same scope and identifier, then a scope error is reported to the programmer. Similarly, if the contextual analyser encounters the \textit{usage} of an identifier during the traversal, it checks that the identifier is already in the type table (i.e., has been previously declared). If the identifier cannot be found in the type table, then a scope error is reported to the programmer. Figure \ref{fig:variable-decl-use} shows snippets of an AST which illustrate the declaration and usage (assignment) of the same variable ``x''. Assuming global scope, in Figure \ref{fig:variable-decl} we would insert an entry with identifier ``x'', type ``INT'' and scope ``global'' into the type table. In Figure \ref{fig:variable-use}, we would lookup ``x'' in the type table to ensure it has been previously declared.

\begin{figure}[h]
	\centering
	\begin{subfigure}[b]{0.3\textwidth}
		\includegraphics[height=2.5cm,width=\linewidth]{images/variable-decl.png}
		\caption{Variable declaration}
		\label{fig:variable-decl}
	\end{subfigure}
	~
	\begin{subfigure}[b]{0.3\textwidth}
		\includegraphics[height=2.5cm,width=\linewidth]{images/variable-use.png}
		\caption{Variable usage}
		\label{fig:variable-use}
	\end{subfigure}
	\caption{AST snippets demonstrating the declaration and usage of a variable ``x''}\label{fig:variable-decl-use}	
\end{figure}

% Need to fix ``...'' stuff and probably better explain these AST snippets

Type checking ensures that every operation has operands of the expected type. The rules the contextual analyser uses to determine if an operation has the ``correct'' operands vary from construct to construct. For example, referring back to Figure \ref{fig:variable-use}, we see the usage of a variable. In this case, we are assigning the value of some expression, represented by ``...'', to the variable ``x''. When the contextual analyser encounters this construct during the traversal, it will retrieve the type of ``x'' from the type table, then traverse the expression to determine its type. The contextual analyser will then check that the type of the expression is the same as the type of ``x''. For example, if we assume that ``x'' is an integer, ``x = 5 + 10'' would be a valid assignment, whereas ``x = `hello world''' would result in a type error.
% Maybe put examples of what expr could be and fix quotes.
% Probably need to sort out usage of variable vs identifier (is a func a variable??)

\subsection{Code Generation}
Upon successful completion of contextual analysis, the generated AST is traversed by a code generator. A code generator translates the source program into a lower level language, such as assembly language or object code. Code generation utilises an auxiliary data structure called an \textit{\textbf{address table}}. Each entry in the address table contains three fields of information about a declared variable: its scope, its identifier and its address. Figure \ref{fig:address-table} demonstrates an example similar to Figure \ref{fig:type-table}. The table reveals how each identifier has been allocated an address in the address space belonging to either local or global variables. Code generation can be broken down into \textit{\textbf{address allocation}} and \textit{\textbf{code selection}}.
...
% Need to better explain code addresses.
% Address allocation bit probably needs more detail but not sure what. Could talk about how addresses are allocated??
% Say like for each construct code generator emits object code, developer must plan what object code will be selected by using code template (this is for code selection).

Address allocation decides the representation and address of each variable in the source program. If the code generator encounters a variable declaration as it is traversing over the AST, it determines a suitable address and inserts that value along with the corresponding identifier and scope into the address table. Determining ``suitable'' addresses is simple in Fun because all variables are of size 1. This means the compiler can simply allocate consecutive addresses to each variable as it is encountered.

Code selection selects and generates the object code. The developer of the compiler must plan what object code should be selected for each construct encountered within the AST. The developer does this by devising a \textit{\textbf{code template}} for each construct in the language, which will be covered in detail in the next section. Upon successful completion of this final phase, the compiler will have often produced an executable program.

\section{Code Templates}
During code generation, the developer of the compiler needs to plan how each construct within the AST should be translated into object code. This can be done using a code template. A code template is often just a high level list of instructions that for each construct, determine how any constituent expressions or commands should be executed, along with specifically stating which assembly language instructions should be emitted.

For example, in Figure \ref{fig:plus} we see an AST snippet that demonstrates the addition of two expressions. In this case, ``expr1'' and ``expr2'' are sub-expressions of ``PLUS''. Consider the case where ``expr1'' is a variable, say ``x''. Naturally, the compiler would need to emit the object code which loads the value of ``x'' into memory before emitting the object code that adds the two expressions together. In fact, this is true for a values including constants, not just variables. Therefore, the code template for the ``PLUS'' construct should state that ``expr1'' and ``expr2'' should be evaluated before we emit any object code to add the expressions together.

\begin{figure}[h]
\centering
\includegraphics[scale=0.7]{images/plus.png}
\caption{AST snippet illustrating the addition of two expressions}
\label{fig:plus}	
\end{figure}

The code template for a PLUS construct might look something like Figure \ref{fig:plus-template}. The figure shows that when arriving at a PLUS, we should execute the code that emits the object code for ``expr1'', then the same for ``expr2'' and finally emit the object code instruction ADD, as shown in blue.

\begin{figure}[h]
\centering
\includegraphics[scale=0.7]{images/plus-template.png}
\caption{Code template for a PLUS construct}
\label{fig:plus-template}	
\end{figure}
