...
In this chapter we define the system architecture of the FunCompiler. That is, we establish the individual system-level components and build a model to specify their organisation/structure, alongside showing the patterns of communication between them.
...
Docker is designed to make it easier to create, deploy, and run applications by using \textit{\textbf{containers}}. A container can be thought of as a miniature, high-speed virtual machine. Prior to obtaining a container, we use an \textit{\textbf{image}} to define a template of the environment we wish to run, this template includes choices for aspects such as the operating system and system/application-level software. We can then ``run'' the image, which creates the corresponding container, as illustrated in Figure \ref{fig:docker1}. It may be helpful to think of a container as an instance of an image.
...
We use a configuration file known as a Dockerfile to automatically build images. A Dockerfile is a text document that expresses the instructions required to carry out tasks ranging from the installation of an operating system to setting the permissions of a file. When we instruct Docker to build an image from a Dockerfile, all the commands contained within are executed and a live container running the environment we specified is created.

Without delving into the specifics, the FunCompiler contains two Dockerfiles which are used to build two different images: one for the web-application, one for the compiler. The exact contents of these Dockerfiles can be found in Appendices \ref{sec:web-dockerfile} and \ref{sec:compiler-dockerfile}. Upon running each image, a container is created which automatically retrieves dependencies and executes any necessary builds as specified in the Dockerfile.

% proof here down
We can use a sub-tool of Docker for running multi-container applications called Docker Compose. Within a YAML file called \textsf{docker-compose.yml} we can define multiple services (or micro-services). For each service, we specify which Dockerfile to build, the ports to expose in order to enable communication, and a command that should be executed once the container is running. In our case, we have two services, one building the web-application image, the other building the compiler image. The command we specify for each container is the command necessary to launch both the Laravel server and the Spark server. By invoking \texttt{docker-compose up}, we execute the contents of \textsf{docker-compose.yml}, which creates both containers and launches both servers, meaning the application is now running and available. To see the full contents of the FunCompiler's \textsf{docker-compose.yml} file refer to Appendix \ref{sec:docker-compose}.

There are a litany of advantages to using Docker but the fundamental point is that if another developer (or perhaps the system admin of a server deploying the application) was to obtain the source code of the FunCompiler, aslong as they had Docker available, they could install every dependency, execute all builds and enable the entire FunCompiler application with the execution of a single command: \texttt{docker-compose up}.
