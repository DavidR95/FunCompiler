...
% Whole section basically needs rewriting
...
The code editor will be the text-area in which users are able to enter code written in the Fun language (FR \textbf{1}). The code editor should be pre-populated with some example code when a user first arrives at the FunCompiler. Ideally, the code editor should also follow some of the semantics of standard programmatic text editors, including syntax highlighting and auto-indention. Notice that in Figure \ref{fig:code-editor-wireframe} two buttons for ``Contextual Analysis'' and ``Code Generation'' have been attached to the bottom of the code editor. These buttons will trigger the transmission of the input code to the compiler and thus initiate the corresponding animation (FR \textbf{2}).
...
\subsection{AST}
The AST of the input program is the foundation of the animation. The AST will be display in a conventional hierarchical tree format where the animator will highlight each node consecutively, corresponding the progress of the compiler (FR \textbf{3, 5}). The highlighted \texttt{PROG} node in Figure \ref{fig:animation-active-wireframe} symbolises the current status of the compiler's traversal. 

\begin{figure}[h]
\centering
\includegraphics[scale=0.6]{images/animation-active-wireframe.png}
\caption{AST wireframe}
\label{fig:animation-active-wireframe}	
\end{figure}

\subsection{Supplementary Information}
We have stated how as the compiler traverses the tree, we would like the application to augment the animation with any extra details that may aid the watcher's understanding. For both compilation phases, this involves a list of descriptions per node in the AST that explain the behaviour of the compiler (FR \textbf{10}) along with the display of the type or address table containing the identifiers encountered at the current point of animation (FR \textbf{11}). The code generation phase also needs to display two other features: the code template (FR \textbf{12}) for each construct and the currently generated object code (FR \textbf{13}). Figure \ref{fig:supp-info-ca} illustrates the type of supplementary information that would be display during contextual analysis, including the current or future logic of the compiler along with the type table. Figure \ref{fig:supp-info-cg} illustrates the same for code generation (swapping the type table for an address table), along with the display of code templates and object code. 
...
\section{Full Design}
In this section we combine the individual components we have developed to create full-size designs for the FunCompiler. We use these full-size designs to act as a ``storyboard'', which is a means a demonstrating a potential workflow through the website. 
...
If the user chooses contextual analysis, they are shown the page illustrated in Figure \ref{fig:full2}. This page displays the AST for the input program, playback controls for the animation, a section to list messages explaining the compiler's logic and the type table. From here the user still has access to the code editor and is free to input a new program and re-select contextual analysis or select code generation. 
