...
During compilation, a source program is parsed into a \textit{\textbf{syntax tree}}. A syntax tree is a hierarchical syntactic representation of a source program; with top-level statements nearer the root, and more deeply nested statements nearer the leaves. We typically consider two types of syntax tree: the \textit{\textbf{parse tree}} (sometimes called a concrete syntax tree) and the \textit{abstract syntax tree} (AST). Translating a source program into a syntax tree means the compiler can more easily reason about the underlying structure and grammatical nature of the program, which is necessary for certain stages of compilation.
...
In this chapter we define the system architecture of the FunCompiler. That is, we extract individual system-level components and build a model to specify the organisation/structure of these components and the patterns of communication between them.

First, we discuss the differences between a monolithic and a micro-service architecture in regards to web-applications. Second, we consider which of these architectures is most applicable to the FunCompiler and how the components of the system will be arranged to fit that model. Finally, from a bottom-up approach we describe the specific technologies that will be used within the FunCompiler, starting with application-level development frameworks and finishing with build tools and packaging systems.
...
For example, consider the scenario of a monolithic online shop. Suppose the vast majority of the website's business logic is written in Python but the authentication service is written in PHP. To force the PHP service to operate within a Python environment not only blatantly violates the basic principles of a separation of concerns, but likely leads to inefficient/insecure methods of communication (e.g., writing to files) and an unscalable, tightly coupled code-base. To solve the issue in this scenario we would need to translate the authentication service from PHP to Python, however, this may be undesirable or simply infeasible.
...
\end{figure}
\section{FunCompiler Architecture}
...
\begin{enumerate}[label=\alph*)]
\item Translate the compiler into the same language used to implement the web-application. 
...
When a user enters a program within the web-application and chooses to animate either contextual analysis or code generation, the web-application makes an API request to the compiler, sending the input program. When the API receives this request, it passes the program as input to the compiler, executes the compilation and sends a response containing the output back to the web-application. The web-application uses the contents of this response to build the corresponding animation. Figure \ref{fig:funinfra0} illustrates this flow of information.
 ...
 \begin{figure}[h]
\centering
\includegraphics[scale=0.6]{images/funinfra0.png}
...
A convenient simplification of the FunCompiler is that the system requires no persistent storage, meaning our system architecture need not include a database. Even without a database, the web-application is required to perform a small amount of server-side (or back-end) work, namely delivering API requests and handling the corresponding responses. We implement this functionality using Laravel. Laravel is an open-source PHP framework intended for the rapid development of web-applications. Like the majority of web-frameworks, Laravel follows a standard model-view-controller (MVC) architectural pattern. 

One might argue that given the limited amount of server-side processing required, a relatively large scale framework such as Laravel is excessive. However, under these circumstances the liklihood of observing anything other than a negligible performance difference between frameworks is extremely low, and whilst it's true that a minimalistic framework (i.e., a Node framework such as Express) could achieve the same functionality, we opt for Laravel due to the extensive experience the developer of this project has with the framework. 

Using Laravel from the beginning also provides some sense of "future-proofing", if the application were required to implement considerably more complex server-side functionality in the future, we already have a framework in place that is capable of handling this, something that may not be possible with a minimalistic framework. 

\subsubsection{Composer}
Within our PHP application were are likely to require several dependencies, including API libraries and Laravel itself. To manage and install these dependencies we use Composer, which is the quuntessential dependency manager for PHP applications.

Composer utilises two important files: composer.json and composer.lock. Within composer.json we can define all the PHP dependencies we require along with the specific versions of those dependencies. By executing composer install, Composer will download and install all the dependencies within compose.json and generate composer.lock, which specifies the version of each dependency installed. After composer.lock has been generated, future invocations of composer install will only download the version specified in composer.lock. 

This versioning system is practical because it means that all developers of a project will install the same dependency versions when executing composer install, assuming composer.lock is committed to version control. If we wish to update the dependencies to more recent versions, we invoke composer update, which updates composer.lock accordingly. 

\subsection{Compiler Technologies}
\subsubsection{ANTLR}
As we know, the compiler for the Fun language is built using ANTLR. ANTLR is a tool which, when given a grammar (which is a file defining the syntax of language), can automatically generate certain components of a standard compiler, such as a lexer and a recursive-descent parser. ANTLR also implements a visitor interface which provides the foundation for executing traversals over the parser-generated syntax tree, allowing the developer to implement a contextual analyser and code generator. 

\subsubsection{Spark}
On its own, the compiler obviously has no way of communicating with the web-application. We form a channel of communication between the two by encapsulating the compiler within an API. The API requires only a single endpoint. The endpoint receives a user submitted input program from the web-application, passes it as input to the compiler, then receives the output from the compiler and finally passes this as a response back to the web-application.

Since the API contains only a single (and relatively simple) endpoint, we look to choose a framework that will allow us to expose an API with as a little overhead as possible, with the only restriction being that it must be Java-based in order to be compatible with the compiler. Given these parameters, we choose a Java micro-framework ideal for creating APIs with minimal boilerplate called Spark (not to be confused with Apache Spark). Spark requires minimal setup and has extremely concise syntax, similar to that of functional languages, meaning we can create an API with extremely small amounts of code.

\subsubsection{Maven}
Maven is the definitive standard in regards to automated build tools for Java projects. To configure a Maven project the developer uses a Project Object Model to define firstly how the software should be built, and secondly the dependencies it requires. This configuration is stored in an XML file named pom.xml. Conveniently, Maven has built-in dependencies for both Spark and ANTLR. 

With the Project Object Model defined, we can simply invoke (in a fashion not too disimiliar to Compoer) mvn package to compile all the Java source files and package the deliverable code into a single JAR file. By executing this JAR file we are booting up the Spark server, which enables the compiler to accept API requests.

\subsection{Docker}
Given the technologies previously listed, our current system infrastructure looks like Figure \ref{fig:maveninfra}. We consider that if we were to attempt to develop the FunCompiler on a different computer, or perhaps deploy the FunCompiler on a server, the host machine would be required to install an abundance of dependencies and manually execute multiple builds. 
...
Similarly to how we used Maven to automate the build process (and dependency installation) of the compiler, we desire to find a technology that will allow us to automate the build process of the entire project. We do this using a tool ever increasing in popularity known as Docker. 

Docker is designed to make it easier to create, deploy, and run applications by using containers. A container can be thought of as a miniture, high-speed virtual machine. Before we can create a container, we must define an image. An image is a template of the environment we wish to run. When we run an image, we are presented with a container, as illustrated in Figure \ref{fig:docker1}.
...
We use a Dockerfile to automatically build images. A Dockerfile is a text document within which we express all the commands a user would have to call via the command line if they were to create the image manually, this includes anything from the installation of an operating system to setting the permissions of a file. When we instruct Docker to build an image from the Dockerfile, all the commands contained within are executed automatically and a live container running the environment we specified is created.

Whilst we won't delve into specifics, the FunCompiler contains two Dockerfiles which are used to build two images: one for the web-application, one for the compiler. The contents of these Dockerfiles can be found in Appendix \ref{sec:Dockeriles}. When the images are run and thus the containers created, they automatically retrieve dependencies and execute any necessary builds.

We then utilise a tool for running multi-container applications called Docker-compose. Within a YAML file called docker-compose.yml we can define several services (refer to \ref{sec:Dockerfiles} for the contents of this file). For each service, we specify which Dockerfile to build, the ports to expose in order to enable communication, and a command that should be executed once the container is running. In our case, we have two services, one building the web-application image, the other building the compiler image. The command we specify for each container is the command necessary to start both the Laravel server and the Spark server. By invoking docker-compose up, we execute the contents of docker-compose.yml, which creates both containers and starts the servers within each one, meaning the application is now running and available.

There are a litany of advantages to using Docker but the take home point is that if another developer (or perhaps the system admin of the server deploying the application) was to obtain the source code of the FunCompiler, aslong as they had Docker installed, they could install every dependency, execute all builds and start up the entire FunCompiler application with the execution of a single command.
