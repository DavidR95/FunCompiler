...
\subsubsection{Maven}
Maven is the definitive standard in regards to automated build tools for Java projects. To configure a Maven project the developer uses a Project Object Model to define firstly how the software should be built, and secondly the dependencies it requires. This configuration is stored in a file called pom.xml. Conveniently, Maven has built-in dependencies for both Spark and ANTLR. 
...
\subsection{Docker}
Given the technologies previously listed, our current system infrastructure looks like Figure \ref{fig:maveninfra}. We consider that if we were to attempt to develop the FunCompiler on a different computer, or perhaps deploy the FunCompiler on a server, the host machine would be required to install an abundance of dependencies. 
...
Similarly to how we used Maven to automate the build process (and dependency installation) of the compiler, we desire to find a technology which will allow us to automate the build process of the entire project. We do this using an emerging tool called Docker. 

Docker is designed to make it easier to create, deploy, and run applications by using containers. A container can be thought of as a miniture, GUI-less virtual machine. Before we can create a container, we must define an image. An image is a template of the environment we wish to run and when we run an image, we get a container, as illustrated in Figure \ref{fig:docker1}.
...
We use a Dockerfile to automatically build images. Within the Dockerfile we express all the commands a user would have to call via the command line if they were to create the image manually, this includes anything from installation of an operating system to setting the permissions of a file. When we run the Dockerfile representing an image, all the commands contained within are executed automatically and a live container running the environment we specified is created.

Whilst we won't delve into specifics, the FunCompiler consists of two images: one for the web-application, one for the compiler. When the containers are created, they automatically retrieve all the dependencies they each need and execute any necessary builds. 

% I think up from here is decent, below deffo needs a bit of work but the content is decent
We then utilise a tool for running multi-container applications called Docker-compose. Using a file called docker-compose.yml, we can create many services. For each service, we specify which container to run, any ports we should expose to enable communication and any commands that should be executed once the container is running. In our case, we have two services, one running the web-application image, the other running the compiler image. The commands we specify are the commands necessary in order to boot up both the Laravel server and the Spark server. Invoking docker-compose up will execute this script, run both containers and start the servers, meaning the entire application is running and available

There are a litany of advantages to using Docker but the take home point is that if another developer (or perhaps the system admin of the server deploying the application) was to obtain the source code for the FunCompiler, aslong as they had Docker installed, they could install every dependency, execute all builds and boot the FunCompiler with the execution of a single command.
